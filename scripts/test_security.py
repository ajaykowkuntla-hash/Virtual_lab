...
def main():
    ...
    tests_e = [
        ...
        ("Admin Create Institution", "POST", "/admin/institutions", admin_token, {"name": "Test Inst", "code": f"TST_{uid}"}, 200),
        ("Student Create Institution", "POST", "/admin/institutions", student_token, {"name": "Test Inst", "code": f"TST_{uid}_2"}, 403),
        ("Faculty Create Institution", "POST", "/admin/institutions", faculty_vance_token, {"name": "Test Inst", "code": f"TST_{uid}_3"}, 403),
        ("No Token Create Institution", "POST", "/admin/institutions", None, {"name": "Test", "code": f"TST_{uid}_4"}, 401),
    ]
    for t in tests_e:
        if not test_endpoint(*t): all_passed = False
