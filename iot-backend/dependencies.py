...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Convert to int inside the try-block so a ValueError triggers a 401, not a 500 crash
...
def get_current_faculty_or_admin(current_user: User = Depends(get_current_user)):
